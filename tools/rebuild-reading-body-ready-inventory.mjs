...
function scores(item){const t=`${item.title} ${item.url}`;const body=item.body.slice(0,4000);return{
  ...
  "travel-reading":((/\/kankocho\//.test(item.url)||(/maff\.go\.jp/.test(item.url)&&(/\/kouryu\/|\/nouhaku/.test(item.url)||/農泊|農山漁村滞在|グリーン.?ツーリズム/.test(item.title)))||(/bunka\.go\.jp/.test(item.url)&&/文化観光|世界遺産|日本遺産|史跡|名勝|文化財を訪|文化施設/.test(t)))&&!/公募|公示|入札|契約|採択|実施結果|組織|予算|税制|SNS|会見/.test(item.title)?10:0),
  ...
  "real-life-documents":(/申請書|届出|手続|ガイド|手引|マニュアル|チェックリスト|記入例|様式|注意事項|利用案内|防災|避難|制度案内|利用方法|安全情報|申請方法|提出方法|登録方法/.test(t)&&!/会見|報道|発表|新着|更新履歴|会議|インタビュー|フォト/.test(item.title)?10:0)
}}
...
const inspected=[];const focusedQueue=[...new Set(targetedRoots)];const focusedSeen=new Set(focusedQueue);const focusedPath=/\/k_ryouri\/|\/kouryu\/|\/nouhaku|\/kankocho\/|\/seisaku\/bunkazai\/|\/policies\/policy\/consumer_|\/policies\/administrative_procedures\/|\/services\//;
...
const queue=[...new Set([...articles.map(x=>x.sourceUrl),...old.map(x=>x.sourceUrl),...roots])];const queued=new Set([...focusedSeen,...queue]);const maxPages=12000;
