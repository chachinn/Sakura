...
(function initializeSakuraBugReport(){
  ...
  function currentRoute(){return String(location.hash||'#home').replace(/^#/,'')||'home';}
  function appVersion(){
    const sw=document.querySelector('script[src*="app.js"]')?.src||'';
    const match=sw.match(/[?&]v=([^&]+)/);
    return match?`app-v${match[1]}`:'v1';
  }
  function userAgentSummary(){return navigator.userAgent.slice(0,500);}
  ...
  function injectStyle(){
    ...
    style.textContent=`
      .sakura-bug-launch{width:100%;display:flex;align-items:center;justify-content:space-between;gap:10px;margin:10px 0 0;padding:12px 13px;border:1px solid var(--color-border);border-radius:14px;background:var(--color-surface);color:var(--color-text);font:inherit;font-weight:850;text-align:left}
  ...
  function buildDialog(){
    ...
    dialog.id='sakura-bug-report-dialog';
    dialog.className='sakura-bug-dialog';
    dialog.setAttribute('aria-labelledby','sakura-bug-title');
    dialog.innerHTML=`
      <section class="sakura-bug-card">
        <header class="sakura-bug-head"><div><span class="section-kicker">Help improve Sakura</span><h2 id="sakura-bug-title">Report a Bug</h2><p>Tell us what went wrong. A screenshot is optional but really helpful.</p></div><button class="sakura-bug-close" type="button" aria-label="Close bug report">×</button></header>
        <form class="sakura-bug-form" id="sakura-bug-form" novalidate>
          <div class="sakura-bug-row"><label>Type<select name="type"><option value="bug">Bug</option><option value="content">Wrong Japanese / content</option><option value="ui">UI / display issue</option><option value="performance">Slow / laggy</option><option value="other">Other</option></select></label><label>Contact email <span class="sakura-bug-optional">Optional</span><input name="contact" type="email" inputmode="email" autocomplete="email" maxlength="160" placeholder="So we can follow up"></label></div>
          <label>Short title<input name="title" maxlength="120" required placeholder="Example: Particle answer is visible"></label>
          <label>What happened?<textarea name="details" maxlength="4000" required placeholder="What did you tap, what appeared, and what was wrong?"></textarea></label>
          <label>What should have happened? <span class="sakura-bug-optional">Optional</span><textarea name="expected" maxlength="2000" placeholder="What did you expect instead?"></textarea></label>
          <label class="sakura-bug-file">Screenshot / photo <span class="sakura-bug-optional">Optional</span><input name="attachment" type="file" accept="image/jpeg,image/png,image/webp,image/heic,image/heif"><small>JPEG, PNG, WebP, HEIC or HEIF · up to 3 MB</small><span class="sakura-bug-file-name" hidden></span></label>
          <label class="sakura-bug-honeypot" aria-hidden="true">Website<input name="website" tabindex="-1" autocomplete="off"></label>
          <p class="sakura-bug-status" role="status" aria-live="polite"></p>
          <div class="sakura-bug-actions"><button class="sakura-bug-cancel" type="button">Cancel</button><button class="sakura-bug-send" type="submit">Send Bug Report</button></div>
        </form>
      </section>`;
    ...
    file.addEventListener('change',()=>{
      ...
      const label=dialog.querySelector('.sakura-bug-file-name');
      if(!chosen){label.hidden=true;label.textContent='';return;}
      label.hidden=false;label.textContent=`${chosen.name} · ${(chosen.size/1024/1024).toFixed(2)} MB`;
    });
  ...
  async function submitReport(event){
    ...
    const send=form.querySelector('.sakura-bug-send');
    ...
    const attachment=data.get('attachment') instanceof File&&data.get('attachment').size?data.get('attachment'):null;
    ...
    try{
      ...
      const payload={
        type,title,details,expected,contact,website:honeypot,
        context:{route:currentRoute(),appVersion:appVersion(),url:location.href,language:navigator.language,userAgent:userAgentSummary(),sentAt:new Date().toISOString()},
        attachment:attachment?{filename:attachment.name,mimeType:attachment.type,size:attachment.size,content:await fileToBase64(attachment)}:null
      };
  ...
  function open(){
    ...
    requestAnimationFrame(()=>d.querySelector('input[name="title"]')?.focus());
  }

  function ensureMenuButton(){
    const hub=document.querySelector('#hub-view');
    const heading=hub?.querySelector('.hub-heading');
    if(!hub||!heading)return false;
    if(document.getElementById('sakura-bug-report-launch'))return true;
    const button=document.createElement('button');
    button.id='sakura-bug-report-launch';
    button.className='sakura-bug-launch';
    button.type='button';
    ...
    const quick=document.getElementById('sakura-hub-mode-switch');
    (quick||heading).insertAdjacentElement('afterend',button);
  ...
  window.SakuraBugReport=Object.freeze({version:1,open,init});
