/* Sakura Reading Garden Production Library v3
   Extends the existing Article/Story runtime with the remaining approved shelves.
   Production indexes are compact; learner body shards load only when opened.
   The source/body-ready evidence archive is never parsed by the browser. */
(function(){
'use strict';
...
const EXCLUDED_SHELVES=new Set(['manga','conversations','diaries','texts','letters','reviews']);
const BODY_CACHE_LIMIT=6;
const PAGE_SIZE=24;
let ready=false,manifest=null,manifestInFlight=null,activeShelf='',activeScreen='',currentIndex=[],currentView=[],currentRecord=null,visibleCount=PAGE_SIZE,translationVisible=false,scrollFrame=0,saveTimer=0,restoredId='';
...
function mergeGenericCompleted(){
 const protectedIds=genericCompleted();
 if(!protectedIds.size)return;
 const value=library(),completed=new Set(Array.isArray(value.completed)?value.completed:[]);
 let changed=false;
...
function markGenericComplete(id){
 ...
 saveProgress(id,activeShelf,document.getElementById('reading-garden-body')?.scrollTop||0,100);
 patchLibraryStrip();renderReader();
}
function setLastReading(record){
 const value=library();
 value.lastReadingId=record.id;value.lastReadingType=record.shelf;value.lastReadingTitle=record.title;value.lastGenericShelf=record.shelf;
 write(LIBRARY_KEY,value);patchLibraryStrip();
}
function toggleSavedViaCore(id){
 const dialog=document.getElementById('reading-garden-dialog');if(!dialog||!id)return;
 const button=document.createElement('button');button.type='button';button.hidden=true;button.dataset.readingSaveArticle=id;dialog.appendChild(button);button.click();button.remove();
 patchLibraryStrip();
}
function patchLibraryStrip(){
 mergeGenericCompleted();
 const strip=document.getElementById('reading-garden-library-strip');if(!strip)return;
...
async function loadManifest(){
 ...
 manifestInFlight=fetchJson(MANIFEST_URL).then(data=>{
  if(data?.totalTarget!==2000||!data?.shelves)throw new Error('Production Reading Garden manifest is invalid.');
  manifest=data;patchHome();return data;
...
async function loadIndex(shelf){
 ...
 const task=fetchJson(`${LIBRARY_BASE}${config.index}?v=1`).then(rows=>{
  if(!Array.isArray(rows)||rows.length!==Number(config.learnerReadyCount))throw new Error(`${shelf} index count does not match the production manifest.`);
  if(rows.some(row=>row?.shelf!==shelf||!row?.id||!row?.pack))throw new Error(`${shelf} index contains an invalid row.`);
  indexCache.set(shelf,rows);return rows;
 }).finally(()=>indexInFlight.delete(shelf));
...
async function loadBodyShard(shelf,pack){
 const url=`${LIBRARY_BASE}${shelf}/${pack}?v=1`;
 if(bodyCache.has(url)){const records=bodyCache.get(url);touchBody(url,records);return records}
 if(bodyInFlight.has(url))return bodyInFlight.get(url);
 const task=fetchJson(url).then(records=>{
  if(!Array.isArray(records)||!records.length||records.length>20)throw new Error(`${shelf}/${pack} is not a valid learner body shard.`);
  if(records.some(record=>record?.shelf!==shelf||!record?.id||!Array.isArray(record?.paragraphs)))throw new Error(`${shelf}/${pack} contains an invalid learner record.`);
  touchBody(url,records);return records;
...
function readyStatus(id,config){
 ...
 if(!readyCount)return'Learner build pending';
 return readyCount===target&&config?.readyForFinal?'Ready now':`${readyCount} / ${target} ready`;
}
function patchHome(){
 ...
 dialog.querySelectorAll('[data-reading-material]').forEach(button=>{
  const id=button.dataset.readingMaterial,config=configFor(id),item=rg.materials?.find(x=>x.id===id);if(!config||!item)return;
  const count=button.querySelector('.reading-material-main small');if(count)count.textContent=`${Number(config.learnerReadyCount||0).toLocaleString()} / ${Number(config.target||item.count).toLocaleString()} ${item.unit}`;
  const status=button.querySelector('.reading-material-status');if(status)status.textContent=readyStatus(id,config);
 });
...
function patchSelection(){
 ...
 const id=dialog.querySelector('[data-reading-material].active')?.dataset.readingMaterial;if(!id||!GENERIC_SHELVES.has(id))return;
 const config=configFor(id),readyCount=Number(config?.learnerReadyCount||0),target=Number(config?.target||0),small=selection.querySelector('small');
...
function ensureScreens(){
 ...
 let reader=document.getElementById('sakura-reading-generic-reader');if(!reader){reader=document.createElement('article');reader.id='sakura-reading-generic-reader';reader.hidden=true;body.appendChild(reader)}
 return{body,browser,reader};
}
...
function showHome(){const screens=ensureScreens();if(screens){screens.browser.hidden=true;screens.reader.hidden=true}activeShelf='';activeScreen='';currentRecord=null;const home=document.getElementById('reading-garden-home');if(home)home.hidden=false;setHeader('Reading Garden','Practice');patchHome();document.getElementById('reading-garden-body')?.scrollTo({top:0,behavior:'auto'})}

function renderBrowser(){
 const browser=document.getElementById('sakura-reading-generic-browser'),config=configFor(activeShelf);if(!browser||!config)return;
 const p=prefs(),query=browser.querySelector('#sakura-generic-search')?.value.trim()||'';
 currentView=currentIndex.filter(row=>(p.level==='all'||row.studyDifficulty===p.level)&&(!query||`${row.title} ${row.titleKana} ${row.titleEnglish} ${row.summary} ${row.sourcePublisher}`.toLowerCase().includes(query.toLowerCase())));
 const offline=offlineState().shelves?.[activeShelf];
 browser.innerHTML=`<section class="reading-browser-hero"><span class="reading-garden-kicker">📖 Reviewed production shelf</span><h2>${esc(config.title)}</h2><p>${Number(config.learnerReadyCount).toLocaleString()} learner readings are currently committed from verified reusable sources. The compact index is loaded now; individual body shards load only when opened.</p></section><div class="reading-browser-toolbar"><input id="sakura-generic-search" type="search" autocomplete="off" placeholder="Search title, Japanese, source…" aria-label="Search ${esc(config.title)}"><div class="reading-browser-levels" role="group">${['all','N5','N4','N3','N2','N1'].map(level=>`<button class="reading-garden-chip${p.level===level?' active':''}" type="button" data-reading-level="${level}">${level==='all'?'All Levels':level}</button>`).join('')}</div></div><div class="reading-browser-offline"><span id="sakura-generic-offline-status">${offline?'✓ This shelf pack was prepared for offline use on this device.':'Prepare only this shelf’s compact index and body shards for offline reading.'}</span><button class="reading-garden-secondary" type="button" data-sakura-download-shelf>${offline?'Refresh Offline Pack':'Download Shelf Pack'}</button></div><div class="reading-browser-meta"><span>${currentView.length.toLocaleString()} reading${currentView.length===1?'':'s'}</span><span>${p.level==='all'?'All levels':p.level}</span></div><div id="sakura-generic-list" class="reading-article-list">${currentView.length?currentView.slice(0,visibleCount).map(cardMarkup).join(''):'<div class="reading-browser-empty">🌸 No readings match these filters.</div>'}</div><button class="reading-garden-secondary reading-load-more" type="button" data-sakura-show-more ${visibleCount>=currentView.length?'hidden':''}>Show More</button>`;
}
...
async function openShelf(shelf){
 await loadManifest();const config=configFor(shelf);if(!GENERIC_SHELVES.has(shelf)||!config?.learnerReadyCount)return;
 activeShelf=shelf;currentRecord=null;visibleCount=PAGE_SIZE;currentIndex=await loadIndex(shelf);showBrowser();renderBrowser();
}
async function openReading(id){
 const row=currentIndex.find(item=>item.id===id);if(!row)return;
 const records=await loadBodyShard(activeShelf,row.pack),record=records.find(item=>item.id===id);if(!record)throw new Error(`${id} is missing from ${row.pack}.`);
 currentRecord=record;translationVisible=false;restoredId='';setLastReading(record);showReader();renderReader();restorePosition();
}
...
function onCapture(event){
 ...
 if(event.type==='click'&&event.target?.closest?.('[data-reading-close]')){activeShelf='';activeScreen='';const screens=ensureScreens();if(screens){screens.browser.hidden=true;screens.reader.hidden=true}const home=document.getElementById('reading-garden-home');if(home)home.hidden=false}
}
function onClick(event){
 ...
 const save=event.target.closest?.('[data-sakura-save-reading]');if(save){event.preventDefault();event.stopPropagation();toggleSavedViaCore(save.dataset.sakuraSaveReading);if(activeScreen==='browser')renderBrowser();else renderReader();return}
 if(event.target.closest?.('[data-sakura-show-more]')){visibleCount+=PAGE_SIZE;renderBrowser();return}
 ...
 if(activeScreen==='browser'&&event.target.closest?.('[data-reading-level]'))setTimeout(()=>{visibleCount=PAGE_SIZE;renderBrowser()},0);
...
function onInput(event){if(activeScreen==='browser'&&event.target?.id==='sakura-generic-search'){visibleCount=PAGE_SIZE;renderBrowser()}}
...
async function init(){
 if(ready)return;const rg=window.SakuraReadingGarden;if(!rg){setTimeout(init,120);return}ready=true;style();ensureScreens();bind();mergeGenericCompleted();
 try{await loadManifest();patchHome()}catch(error){console.warn('Reading Garden production manifest is unavailable; the existing Article/Story runtime remains available.',error)}
}
window.SakuraReadingQuality=Object.freeze({version:3,legacyCapRetired:true,loadManifest,openShelf,openReading,init,get learnerReadyCount(){return Number(manifest?.learnerReadyCount||0)},get activeShelf(){return activeShelf}});
init();
}());
