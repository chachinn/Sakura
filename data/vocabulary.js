...
(function initializeSakuraVocabularyLoader() {
    ...
    function validateVocabularyRecords(records, level) {
        ...
        const loadedIds = new Set([...loadedByLevel.entries()].filter(([loadedLevel]) => loadedLevel !== level).flatMap(([, items]) => items).map(item => item.id));
        const loadedIdentities = new Set([...loadedByLevel.entries()].filter(([loadedLevel]) => loadedLevel !== level).flatMap(([, items]) => items).map(item => `${item.word}\u0000${item.kana}`));
        const requiredStrings = ["id", "type", "word", "kana", "romaji", "meaning", "jlpt", "exampleSentence", "exampleTranslation", "notes"];
        ...
        records.forEach((record, index) => {
            ...
            ids.add(record.id);
            const identity = `${record.word}\u0000${record.kana}`;
            if (identities.has(identity) || loadedIdentities.has(identity)) errors.push(`${label} duplicate word+kana ${record.word} / ${record.kana}.`);
