...
(function initializeSakuraKanjiLoader() {
    ...
    async function readKanjiFile(file) {
    ...
    function loadKanjiLevel(level) {
        ...
        const fetchRequest = (async () => {
            const records = await readKanjiFile(files[validLevel]);
            if (records.some(record => record?.jlpt !== validLevel)) throw new Error(`${files[validLevel]} contains a record outside ${validLevel}.`);
    ...
    function loadAllKanji() { return loadKanjiLevels(levels); }
    function getLoadedKanji() { return levels.flatMap(level => loadedByLevel.get(level) || []); }
    function getLoadedKanjiLevels() { return levels.filter(level => loadedByLevel.has(level)); }
    ...
    window.SakuraKanjiLoader = Object.freeze({
        levels, files, loadKanjiLevel, loadKanjiLevels, loadAllKanji,
        getLoadedKanji, getLoadedKanjiLevels, getStartupKanjiLevels
    });
